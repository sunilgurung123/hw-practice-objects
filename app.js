...
*/
// used filter method to get all the starter with true value from pokemon arrays
// used push method to add true element into new array (game.party)
const starters = pokemon.filter(pokemons => pokemons.starter === true);
game.party.push(...starters); // used spread operator to add all element
...
const threeMoreItems = [
    pokemon.find(pokemons => pokemons.type === 'water'),
    pokemon.find(pokemons => pokemons.type === 'grass'),
    pokemon.find(pokemons => pokemons.type === 'normal')
];

game.party.push(...threeMoreItems);
// console.log(game.party);
...
for (let i = 0; i < game.party.length; i++) {
  ...
  const evolvedNumber = evolutionMap[currentPoke.number];
  
  if (evolvedNumber) {
    const evolvedForm = pokemon.find(pokemons => pokemons.number === evolvedNumber);
...
const seel = pokemon.find(pokemons => pokemons.name === "Seel");
...
game.catchPokemon = function(pokemonObj) {
    this.party.push(pokemonObj);
    const seel = this.items.find(item => item.name === "seel");
    if (seel) {
    pokeball.quantity -= 1;
  }
};

const meowth = pokemon.find(pokemons => pokemons.name === "Meowth");
